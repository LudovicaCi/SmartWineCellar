...
#define SENSOR_TYPE "humidity_whitewine"

/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "humidity_whitewine"
...
PROCESS(humidity_whitewine_server, "Humidity White Wine Server");
AUTOSTART_PROCESSES(&humidity_whitewine_server);
...
PROCESS_THREAD(humidity_whitewine_server, ev, data){
	PROCESS_BEGIN();
	...
	LOG_INFO("Starting Humidity White Wine CoAP-Server\n");
	coap_activate_resource(&res_dehumidifier_whitewine_system, "humidity_whitewine/dehumidifier"); 
	coap_activate_resource(&res_humidity_whitewine_sensor, "humidity_whitewine/humidity");
	...
	while(1) {
		...
		if(ev == PROCESS_EVENT_TIMER && data == &simulation_timer) {
			res_humidity_whitewine_sensor.trigger();	
