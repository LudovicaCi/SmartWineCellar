...
#define SENSOR_TYPE "humidity_redwine"

/* Log configuration */
#include "sys/log.h"
#define LOG_MODULE "humidity_redwine"
...
PROCESS(humidity_redwine_server, "Humidity Red Wine Server");
AUTOSTART_PROCESSES(&humidity_redwine_server);
...
PROCESS_THREAD(humidity_redwine_server, ev, data){
	PROCESS_BEGIN();
	...
	LOG_INFO("Starting Red Wine CoAP-Server\n");
	coap_activate_resource(&res_dehumidifier_redwine_system, "humidity_redwine/dehumidifier"); 
	coap_activate_resource(&res_humidity_redwine_sensor, "humidity_redwine/humidity");
	...
	while(1) {
		...
		if(ev == PROCESS_EVENT_TIMER && data == &simulation_timer) {
			res_humidity_redwine_sensor.trigger();	
