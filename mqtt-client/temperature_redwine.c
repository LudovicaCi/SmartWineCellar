...
#define LOG_MODULE "temperature-redwine-sensor"
...
PROCESS_NAME(process_for_temperature_redwine_sensor);
AUTOSTART_PROCESSES(&process_for_temperature_redwine_sensor);
...
PROCESS(process_for_temperature_redwine_sensor, "Temperature redwine sensor process");
...
static void mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data) {
	switch(event) {
		...
		case MQTT_EVENT_DISCONNECTED: 
		{
			...
			process_poll(&process_for_temperature_redwine_sensor);
			break;
		}
...
PROCESS_THREAD(process_for_temperature_redwine_sensor, ev, data) {
	...
	// Broker registration					 
	mqtt_register(&conn, &process_for_temperature_redwine_sensor, client_id, mqtt_event,
                     MAX_TCP_SEGMENT_SIZE);
	
	printf("Mqtt register\n");
	...
	while(1) {
		...
		if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) ||
			...
			if(state==STATE_CONNECTED) {
				...
				strcpy(sub_topic,"temperature_regulator_red");
			...
			if(state == STATE_SUBSCRIBED) {	
				sprintf(pub_topic, "%s", "current_temperature_red");
				...
				if (increase_temperature || decrease_temperature) {
					...
					if((rand()%10) < 6) { // 60% chance that the temperature will change
						variation = (rand()%7)-3; // a value in [-3, 3]
            ...
            btn = (button_hal_button_t*) data;
            printf("Bottone premuto per %u seconds\n",btn->press_duration_seconds);
	    int temp_diff = (btn->press_duration_seconds)/3;
            temperature -= temp_diff;
	    printf("Temperatura diminuita di %d gradi", temp_diff);
