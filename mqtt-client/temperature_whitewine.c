...
#define LOG_MODULE "temperature-whitewine-sensor"
...
PROCESS_NAME(process_for_temperature_whitewine_sensor);
AUTOSTART_PROCESSES(&process_for_temperature_whitewine_sensor);
...
PROCESS(process_for_temperature_whitewine_sensor, "Temperature whitewine sensor process");
...
static void pub_handler(const char *topic, uint16_t topic_len, const uint8_t *chunk, uint16_t chunk_len) {
	...
	if(strcmp(topic, "temperature_regulator_white") == 0) {
...
static void mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data) {
	switch(event) {
		...
		case MQTT_EVENT_DISCONNECTED: 
		{
			...
			process_poll(&process_for_temperature_whitewine_sensor);
			break;
		}
...
PROCESS_THREAD(process_for_temperature_whitewine_sensor, ev, data) {
	...
	// Broker registration					 
	mqtt_register(&conn, &process_for_temperature_whitewine_sensor, client_id, mqtt_event,
	...
	while(1) {
		...
		if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) ||
			...
			if(state==STATE_CONNECTED) {
				...
				strcpy(sub_topic,"temperature_regulator_white");
			...
			if(state == STATE_SUBSCRIBED) {	
				sprintf(pub_topic, "%s", "current_temperature_white");
